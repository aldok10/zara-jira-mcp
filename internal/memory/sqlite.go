...
import (
	...
	_ "github.com/mattn/go-sqlite3" // SQLite driver registration (init)
...
func NewSQLiteStore(dbPath string) (*SQLiteStore, error) {
	...
	if err := s.migrateDeep(); err != nil {
		return nil, fmt.Errorf("migrate deep: %w", err)
	}
...
// SQLDB returns the underlying *sql.DB for use by subsystems that need direct access
// (e.g. the agent session store).
...
func (a *rawDBAdapter) Query(query string, args ...any) (domain.Rows, error) {
	//nolint:rowserrcheck // returned as domain.Rows, caller decides
...
// Backup creates a consistent backup of the SQLite database.
// Uses the sqlite3 backup API for consistent, online backups.
func (s *SQLiteStore) Backup(ctx context.Context, destPath string) error {
	// SQLite backup API requires access to the raw driver connection.
	// For now, return not implemented error.
	// TODO: Implement using sqlite3 backup API when needed.

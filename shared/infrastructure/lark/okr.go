...
func (c *OKRClient) CreateProgressRecord(ctx context.Context, krID string, content string) error {
	...
	payload := fmt.Sprintf(`{"target_id":%q,"target_type":2,"content":{"blocks":[{"type":"paragraph","content":[{"type":"text","text":%q}]}]}}`, krID, escaped)
...
func (c *OKRClient) get(ctx context.Context, url string) ([]byte, error) {
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("lark OKR API returned status %d", resp.StatusCode)
	}
...
func (c *OKRClient) getTenantToken(ctx context.Context) (string, error) {
	...
	payload := fmt.Sprintf(`{"app_id":%q,"app_secret":%q}`, c.appID, c.appSecret)

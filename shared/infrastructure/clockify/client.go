...
func (c *Client) GetTimeEntries(ctx context.Context, start, end time.Time) ([]TimeEntry, error) {
	...
	body, err := c.doPost(ctx, url, fmt.Sprintf(`{"dateRangeStart":%q,"dateRangeEnd":%q,"detailedFilter":{"page":1,"pageSize":200}}`,
...
func (c *Client) GetSummaryReport(ctx context.Context, start, end time.Time) (map[string]map[string]time.Duration, error) {
	...
	body, err := c.doPost(ctx, url, fmt.Sprintf(`{"dateRangeStart":%q,"dateRangeEnd":%q,"summaryFilter":{"groups":["USER","PROJECT"]}}`,
		start.Format(time.RFC3339), end.Format(time.RFC3339)))
	if err != nil {
		return nil, err
	}
